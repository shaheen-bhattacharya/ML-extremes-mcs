...
class GenerateTrainData:
    ...
    Attributes:
        ...
        mask_var (str): Variable name in mask file.
                        Defaults to ``cloudtracknumber``.
    ...
    def make_years(self):
        ...
        """
        return pd.date_range(start=str(self.year_start)+'-01-01',
                             end=str(self.year_end)+'-01-01',
                             freq='AS-JAN').year
    ...
    def slice_grid(self, data_mask, data, lat='latitude', lon='longitude'):
        ...
        Args:
            ...
            lat (str): Latitude dimension name for variable data.
                       Defaults to ``latitude`` for era5.
            lon (str): Longitude dimension name for variable data.
                       Defaults to ``longitude`` for era5.
        ...
        data = data.isel(
            latitude=slice(np.where(data[lat].values == lat1_bnd)[0][0],
    ...
    def regrid_mask(self, ds, method='nearest_s2d', 
        ...
        ds_out = xe.util.grid_2d(
            lon0_b=lon0_bnd-offset, lon1_b=lon1_bnd+offset, d_lon=dcoord,
    ...
    def generate_files(self, p_height=None, dict_freq='3H', file_indx=None):
        ...
        if file_indx:
            ...
            indx_array = dict(itertools.islice(indx_array.items(), file_indx,
                                               len(indx_array), 1))
    ...
    def generate_masks(self, dict_freq='3H', file_indx=None):
        ...
        if file_indx:
            ...
            indx_array = dict(itertools.islice(indx_array.items(),
                                               file_indx,
                                               len(indx_array), 1))

        for indx_val, indx_dt in indx_array.items():

            mask = self.open_mask_file(year=indx_dt.strftime('%Y'),
                                       month=indx_dt.strftime('%m'),
                                       day=indx_dt.strftime('%d'),
                                       hour=indx_dt.strftime('%H'))
            ...
            tmpmask.to_netcdf(
                f"{self.main_directory}/dl_files/{dict_freq}/mask/mask_{self.msk_var}_ID{indx_val}.nc")

        print("Job complete!")
        return

    def generate_train_stats(self, analysis_variable, dict_freq='3H', lat='latitude', lon='longitude', author=None):
        """
        ...
        Args:
            ...
            dict_freq (str): Files of specific hourly time spacing.
                             Defaults to ``3H`` for 3-hourly.
